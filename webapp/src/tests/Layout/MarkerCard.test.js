...
describe("MarkerCard", () => {
	const marker = {
		...
		score: [{ score: 3 }, { score: 4 }, { score: 5 }],
		rating: 4.0,
		comments: [
			{ author: "Tester", comment: "Test comment 1", date: "2023-04-12" },
			{
				author: "Tester2",
	...
	test("The marker card is rendered", async () => {
		const { getByText } = render(
			<MarkerCard marker={marker} session={{ session: mockSession }} />
		);
	...
	test("displays the rating", async () => {
		const { getByLabelText } = render(<MarkerCard marker={marker} />);
		expect(getByLabelText("star"));
	});

	test("displays the upload image", async () => {
		const { getByLabelText } = render(<MarkerCard marker={marker} />);
		expect(getByLabelText("uploadFirebase"));
	});
});
