...
const WriteToPod = () => {
	...
	async function insertThing() {
		...
		const thing = buildThing(createThing({ name: "Prueba1" }))
			.addStringNoLocale(SCHEMA_INRUPT.name, "Ejemplo inserción de datos pods")
			.addStringNoLocale(SCHEMA_INRUPT.name, "x = 54")
			.addStringNoLocale(SCHEMA_INRUPT.name, "y = -4.645464564")
			.addStringNoLocale(SCHEMA_INRUPT.name, "z = 200")
			.build();
